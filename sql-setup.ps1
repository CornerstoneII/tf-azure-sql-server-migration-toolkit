# SQL Server VM Setup Script - Clean Version
param(
...
try {
    ...
    foreach ($credFormat in $credentialFormats) {
        try {
            ...
            Write-Log "Authentication failed with $credFormat" "WARNING"
        }
    }
    ...
    try {
        ...
        Write-Log "PowerShell mount failed, trying net use command as fallback..." "WARNING"
    ...
    if ($localBackupFiles.Count -eq 0) {
        ...
        for ($i = 1; $i -le 5; $i++) {
            try {
                ...
                Write-Log "SQL connectivity test failed" "WARNING"
            }
